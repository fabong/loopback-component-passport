...
module.exports = function(UserIdentity) {
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    /*
      Option to disable user model validation if there is no need to validate user schema
      It's also a fix for the following issue:
        First, userModel.findOrCreate function has to use an atomic operation (for example with loopback-connector-mongodb, it is when enableOptimizedFindOrCreate option is activated).
        Then the case is if a user exists in database and he hasn't logged in with third-party yet, the function will return a none expected error about uniqueness email validation with the found user.

      For more information, see:
      - https://github.com/strongloop/loopback-component-passport/issues/123
      - https://github.com/strongloop/loopback-component-passport/issues/160
      - https://github.com/strongloop/loopback-datasource-juggler/issues/529
      - https://github.com/strongloop/loopback-connector-mongodb/issues/205
    */
    var validateUser = options.validateUser || options.validateUser === undefined;
    ...
    userIdentityModel.findOne({where: {
      ...
      userModel.findOrCreate({where: query}, userObj, {validate: validateUser}, function(err, user) {
        if (err) {
          return cb(err);
        }
        var date = new Date();
